...
//uniform vec4 texColor = vec4(0);
//uniform float whiteNoise = 9999.0;
...
void fragment(){
    ...
    samplePosition.y = samplePosition.y+(rand(vec2(TIME))-.15)/9.0;
    // Slightly add color noise to each line
    texColor = texColor + (vec4(-0.5)+vec4(rand(vec2(FRAGCOORD.y,TIME)),rand(vec2(FRAGCOORD.y,TIME+1.0)),rand(vec2(FRAGCOORD.y,TIME+2.0)),2))*0.1;
   
    // Either sample the texture, or just make the pixel white (to get the staticy-bit at the bottom)
    whiteNoise = rand(vec2(floor(samplePosition.y*600.0),floor(samplePosition.x*100.0))+vec2(TIME,0));
